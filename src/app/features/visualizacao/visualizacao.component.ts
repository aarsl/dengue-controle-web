import { Component, OnInit, signal, ElementRef, ViewChild, ChangeDetectorRef } from '@angular/core';
...
export class VisualizacaoComponent implements OnInit {
	...
	carregarAtividades(): void {
		...
		this.route.queryParams.subscribe(queryParams => {
			const atividadeId = queryParams['atividadeId'] ? +queryParams['atividadeId'] : null;
			if (atividadeId) {
	...
	private salvarDadosImovel(): void {
		...
		this.localVistoriaService.atualizar(this.localAtual.id, localDTO).subscribe({
			...
			error: (errorResponse) => {
				...
				this.toast.error('Erro ao atualizar: ' + errorResponse.message);
				this.restaurarLocalBackup();
			}
		});
	}
	...
	private restaurarLocalBackup(): void {
		if (this.localBackup && this.localAtual) {
			this.localAtual = { ...this.localBackup };
			this.localBackup = undefined;
